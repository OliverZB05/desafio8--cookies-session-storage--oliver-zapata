...
try {
    ...
    // Validar si el usuario ha ingresado un valor para el correo electrónico y la contraseña
    ...
    // Verificar si la contraseña ingresada es correcta utilizando bcrypt.compare
    const isPasswordCorrect = await bcrypt.compare(password, user.password);
    if (!isPasswordCorrect) return res.status(400).send({ status: 'error', error: 'Contraseña incorrecta' });
